...
The reason we have the time penalty for human touching/intervening/interacting is that sometimes robots get stuck, and teams don't want to restart their run. Or, sometimes teams want to upload new code to their robot in the middle of the run. Although that clearly reduces the autonomy of the robot, the reality is that no robot in real life is really truly autonomous.
...
This year, since our Arduinos have Bluetooth and Wifi capabilities, you *can* run an AI system on your computer and have it send commands to your robot. However, the system must still be strictly autonomous and every human intervention will incur a 1-min penalty.
...
Now we can put the challenges together. You will need to figure out a method for transitioning between challenges. We may have special markers that can help you determine the beginning or end of a challenge, as demonstrated in the TA model tournament. Create your own model transistions.
