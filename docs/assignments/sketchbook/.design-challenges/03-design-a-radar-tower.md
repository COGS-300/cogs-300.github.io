...
Using long-distance range sensors like an ultrasonic, build a [sonar](https://en.wikipedia.org/wiki/Sonar) tower (which is like a [radar tower](https://en.wikipedia.org/wiki/Radar), but with sound).
