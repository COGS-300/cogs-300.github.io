...
## Introduction
As you saw in the [Electricity 101](/docs/concepts/teaching/activities/activity-00-electricity.mdx) activity, we can make a `HIGH` or `LOW` signal by setting a voltage to `5V` or `0V`. The threshold for `HIGH` is arbitrary (but has good engineering reasons behind it). The measurement for `5V` is relative to something we define as `0V`, so we need an electrically neutral material to reference the voltage to, such as the ground. 
...
Digital signals encode information with `HIGH` and `LOW` voltages, which are treated as truth values. If a [human computer](https://en.wikipedia.org/wiki/Computer_(occupation) (that means a person who does computation for a living, like an accountant), then anything computable by a human is machine-computable. The [Church-Turing thesis](https://plato.stanford.edu/entries/church-turing/) is often used by philosophers to question the nature of the universe and cognition. Can everything be translated effectively into computable digital signals? Discuss.