...
## Introduction
As you saw in the [Electricity 101](/docs/concepts/teaching/activities/00-electricity.mdx) activity, we can make a `HIGH` or `LOW` signal by setting a voltage to `5V` or `0V`. The threshold for `HIGH` is arbitrary (but has good engineering reasons behind it). The measurement for `5V` is relative to something we define as `0V`, so we need an electrically neutral material to reference the voltage to, such as the ground. 
